clear; clc;
%% Checkpoint Check
if exist( 'checkpointGS_first.mat','file' ) % If a checkpoint file exists, load it
    load('checkpointGS_first.mat')
end
...
while Max > 10^-6
    ...
    if mod(Count,1000)==0   %Save checkpoint file every 1000 iterations
        save('checkpointGS_first.mat'); %Save the file
    end     %Close if loop
...
xlabel('x') %Label the x-axis
set(h,'linestyle','none');  %Remove the gridlines
if exist( 'checkpointGS_first.mat','file' )
    delete('checkpointGS_first.mat');    %Delete checkpoint file once everything is complete
end