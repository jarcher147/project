clear; clc;
%% Checkpoint Check
if exist( 'checkpointGS_zero.mat','file' ) % If a checkpoint file exists, load it
    load('checkpointGS_zero.mat')
end
...
while Max > 10^-6
    ...
    if mod(Count,1000)==0   %Save checkpoint file every 1000 iterations
        save('checkpointGS_zero.mat'); %Save the file
    end     %Close if loop
...
xlabel('x') %Label the x-axis
set(h,'linestyle','none');  %Remove the gridlines
if exist( 'checkpointGS_zero.mat','file' )
    delete('checkpointGS_zero.mat');    %Delete checkpoint file once evertything is complete
end
