clear; clc;
%% Checkpoint Check
if exist( 'checkpointSOR_first.mat','file' ) % If a checkpoint file exists, load it
    load('checkpointSOR_first.mat')
end
...
while Max > 10^-6
    for k=2:Ny+1    %All y points not on the boundary
        ...
        %BC for x=ax
        U(1,k)=(Dx*(U(1,k-1)+U(1,k+1))+Dy*U(2,k))/(2*Dx+Dy);  %Compute boundary value at x=ax
        %BC for x=bx
        F=sin(pi*(bx-ax)/Lx)*cos((0.5*pi)*(2*(y-ay)/Ly)+1);  %Define F(x,y) for the particular j,k point
        U(End,k)=((Dx*(U(End,k-1)+U(End,k+1))+Dy*U(End-1,k))+(Dx*Dy*F))/(2*Dx+Dy); %Compute boundary value at x=bx
    ...
    if mod(Count,1000)==0   %Save checkpoint file every 1000 iterations
        save('checkpointSOR_first.mat'); %Save the file
    end     %Close if loop
...
xlabel('x') %Label the x-axis
set(h,'linestyle','none');  %Remove the gridlines
if exist( 'checkpointSOR_first.mat','file' )
    delete('checkpointSOR_first.mat');    %Delete checkpoint file once everything is complete
end
